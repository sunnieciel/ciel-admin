...
import {clearUserLoginLog, listData} from "../../../libs/api-admin";
...
const MainPage = ({query, setQuery, setShowType, setId}) => {
    ...
    const handleClear = async () => {
        if (confirm('确认清空?')) {
            const {code} = await clearUserLoginLog()
            if (code === 0) {
                toast.success('操作成功')
                mutate()
            }
    ...
    return <>
        <PageInfo>
            <div className={'cell p-3 flex-center'}>
                <button className={'btn-danger   ml-6 mr-auto'} onClick={handleClear}>清空</button>
